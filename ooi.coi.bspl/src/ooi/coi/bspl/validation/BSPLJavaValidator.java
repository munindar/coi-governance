...
public class BSPLJavaValidator extends AbstractBSPLJavaValidator {
  ...
  public void checkRoleName(Role type) {
    if (!Character.isUpperCase(type.getName().charAt(0))) {
      this.warning("Role name should begin with an uppercase letter", BSPLPackage.Literals.ROLE__NAME,
          BSPLJavaValidator.INVALID_ROLE_NAME, type.getName());
    }
  }
  ...
  public void checkParameterName(Parameter parameter) {
    ...
    if (!Character.isLowerCase(pName.charAt(0)) && !Character.isUpperCase(pName.charAt(pName.length() - 1))) {
      this.warning("Parameter name should begin with a lowercase letter or end with an uppercase letter",
  ...
  public void checkParameDeclOut(ParamDecl pDecl) {
    if (pDecl.getAdornment() == kAdornment.OUT) {
      ...
      this.warning("An OUT parameter must match with at least one reference with OUT in it",
    ...
    if (pDecl.getAdornment() == kAdornment.IN) {
      ...
      for (EObject aRef : allRefs) {
        if (this.usesParam(aRef, pDecl.getParam(), kAdornment.OUT)
          ...
          this.warning("An IN parameter must be IN throughout", BSPLPackage.Literals.PARAM_DECL__PARAM,
