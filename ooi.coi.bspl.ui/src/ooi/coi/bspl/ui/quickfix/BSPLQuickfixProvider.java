...
public class BSPLQuickfixProvider extends DefaultQuickfixProvider {
  ...
  public void fixRoleName(final Issue issue, IssueResolutionAcceptor acceptor) {
    acceptor.accept(issue, "Capitalize role name", "Capitalize role name '"
  ...
  public void fixParameterName(final Issue issue,
  ...
  public void createMissingRole(final Issue issue,
      IssueResolutionAcceptor acceptor) {
    final String linkText = issue.getData()[0];
    acceptor.accept(issue, "Create role '" + linkText + "'", "Create role '"
        + linkText + "'", null,
    // exemplary semantic modification
        new ISemanticModification() {
          @Override
          public void apply(final EObject element, IModificationContext context) {
            BSPLQuickfixProvider.this.createRole((Role) element, linkText);
          }
        });
  ...
  public void createMissingPublicParameter(final Issue issue,
      IssueResolutionAcceptor acceptor) {
    final String linkText = issue.getData()[0];
    acceptor.accept(issue, "Create public parameter '" + linkText + "'",
        "Create public parameter '" + linkText + "'", null,
        new ISemanticModification() {
          @Override
          public void apply(final EObject element, IModificationContext context) {
            BSPLQuickfixProvider.this.createPublicParameter((Parameter) element
                .eContainer().eContainer(), linkText);
          }
        });
    this.createLinkingIssueResolutions(issue, acceptor);
  }
  ...
  protected boolean createRole(Role aRole, String name) {
    ...
    return this.addRoleAsSibling(aRole, newRole);
  }
  ...
  protected boolean createPublicParameter(Parameter aParameter, String name) {
    ...
    return this.addPublicParameterAsSibling(aParameter, newParameter);
  }

  protected boolean addPublicParameterAsSibling(Parameter aParameter,
      ParamDecl newParameter) {
